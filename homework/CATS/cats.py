...
def pick(paragraphs, select, k):
    ...
    """
    # BEGIN PROBLEM 1
    qualified = [str for str in paragraphs if select(str)]
    if k >= len(qualified):
        return ""
    else:
        return qualified[k]
...
def about(subject):
    ...
    # BEGIN PROBLEM 2
    def specific_about(paragraph):
        lower_paragraph = remove_punctuation(lower(paragraph))
        words_from_paragraph = split(lower_paragraph)
        for sub in subject:
            for para in words_from_paragraph:
                if sub == para:
                    return True
        return False

    return specific_about
    # END PROBLEM 2
...
def accuracy(typed, source):
    ...
    for i in range(min(typed_len, source_len)):
        if typed_words[i] == source_words[i]:
            correct_count += 1
    res = correct_count / typed_len * 100.0
    return res
    # END PROBLEM 3
...
def autocorrect(typed_word, word_list, diff_function, limit):
    ...
    #     if diff == min_diff:
    #         min_word_list.append(str)
    #     if diff < min_diff:
    #         min_word_list = []
    #         min_word_list.append(str)
    #         min_diff = diff
    # if min_diff > limit or len(min_word_list) == 0:
    #     return typed_word
...
def furry_fixes(typed, source, limit):
    ...
    def helper(index, curr_score):
        ...
        if typed[index] != source[index]:
            curr_score += 1
        return helper(index + 1, curr_score)
...
def report_progress(typed, source, user_id, upload):
    ...
    for i in range(len(typed)):
        if typed[i] == source[i]:
            ...
            break
    score = correct_count / len(source)
    dic = {}
    dic["id"] = user_id
    dic["progress"] = score
    upload(dic)
...
def time_per_word(words, timestamps_per_player):
    ...
    for time_sgpl in tpp:
        times.append(
            [time_sgpl[i + 1] - time_sgpl[i] for i in range(len(time_sgpl) - 1)]
        )
    # END PROBLEM 9
...
def fastest_words(words_and_times):
    ...
    for i in word_indices:
        times_player = [times[j][i] for j in player_indices]
        min_player_index = times_player.index(min(times_player))
